...
async function renderOutput(finalMap, synMap, generateSyn, compress = true) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const hdr = idxPos; idxPos += 8;       // reserve 8-byte header slot
  ...
  if (generateSyn && synMap && synMap.size > 0) {
    ...
    addLog(i18nText('logSynonymFile', 'Synonym file: {count} entries.', { count: synCount }));
  }
  ...
  if (compress) {
    addLog(i18nText('logCompressingDict',
    ...
    const dictDzBytes = await compressDictzip(dictBytes, (done, total) => {
      if (done % 200 === 0 || done === total) {
        addLog(i18nText('logCompressingProgress',
          'Compression: {done}/{total} chunks…', { done, total }));
      }
    });
    const ratio = dictTotalSize > 0
      ? Math.round((1 - dictDzBytes.length / dictTotalSize) * 100)
      : 0;
    addLog(i18nText('logCompressingDone',
  ...
  if (synCount > 0) {
    ...
    synWrap.style.cssText = 'display:inline-flex;flex-direction:column;align-items:center;vertical-align:top;margin-top: 5px;';
    ...
    badge.textContent = i18nText('validatorSynonymBadge', '{count} synonyms', { count: synCount });
  ...
  addLog(i18nText('logOutputReady',
